...
import type {
  ...
  BookProblem,
  BookRequest,
} from "@/core/teacher/books/books.types";
import {
  BookOpen,
  CheckCircle,
  Clock,
...
export default function BooksPage() {
  ...
  const [viewingProblems, setViewingProblems] = useState<Book | null>(null);
  ...
  const allBookRequests = getBookRequests();
  ...
  const prefillRandomISBN = () => {
    ...
    if (randomBook) setSearchTerm(randomBook.isbn);
  };
  ...
  const getSelectedProblemsFromBook = () => {
    ...
    return viewingProblems.problems.filter((p) =>
  ...
  const uniqueGrades = Array.from(
    new Set(allBooks.map((book) => book.grade)),
  ).sort();
  ...
  return (
    <div className="flex-1 space-y-4 p-4 pt-6 md:p-8">
            ...
            <div className="grid gap-4 py-4">
              <div className="grid grid-cols-4 items-center gap-4">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setRequestForm({ ...requestForm, isbn: e.target.value })
                  }
        ...
        <TabsContent value="catalog" className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center space-x-2">
              ...
              <Button
                variant={
                  availabilityFilter === "unavailable" ? "default" : "outline"
                }
                size="sm"
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center space-x-2">
              ...
              <Button
                variant={
                  scannedFilter === "not-scanned" ? "default" : "outline"
                }
                size="sm"
        ...
        <TabsContent value="requests" className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <div className="flex items-center space-x-2">
              ...
              >
                Pending (
                {allBookRequests.filter((r) => r.status === "pending").length})
              </Button>
              <Button
                variant={
              ...
              >
                Approved (
                {allBookRequests.filter((r) => r.status === "approved").length})
              </Button>
              <Button
                variant={
                  requestStatusFilter === "ordered" ? "default" : "outline"
                }
                size="sm"
                onClick={() => setRequestStatusFilter("ordered")}
              >
                Ordered (
                {allBookRequests.filter((r) => r.status === "ordered").length})
              </Button>
              <Button
                variant={
                  requestStatusFilter === "received" ? "default" : "outline"
                }
                size="sm"
                onClick={() => setRequestStatusFilter("received")}
              >
                Received (
                {allBookRequests.filter((r) => r.status === "received").length})
              </Button>
              <Button
                variant={
              ...
              >
                Rejected (
                {allBookRequests.filter((r) => r.status === "rejected").length})
              </Button>
            </div>
          </div>
        ...
        <TabsContent value="search" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Search for books in our catalog using ISBN number
              </CardDescription>
            </CardHeader>
                        ...
                        <CardHeader>
                          <div className="flex items-start justify-between">
                            ...
                            <div className="flex flex-col gap-1">
                              <Badge
                                variant={
                                  book.available ? "default" : "secondary"
                                }
                              >
                                {book.available ? "Available" : "Unavailable"}
                              </Badge>
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            {book.isScanned && book.problems && (
                              <p>
                                ...
                                {book.problems.length} available
                              </p>
                            )}
                          </div>
                          <div className="mt-4 flex space-x-2">
                            ...
                            {book.isScanned && book.problems && (
                              <Button
                                variant="outline"
                                onClick={() => handleViewProblems(book)}
                              >
          ...
          {viewingBook && (
            <div className="max-h-[60vh] space-y-6 overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <p className="font-mono text-sm">{viewingBook.isbn}</p>
                  </div>
                  <div>
                    <h4 className="text-sm font-semibold text-muted-foreground">
                      Publisher
                ...
                <div className="space-y-3">
                  ...
                  <div>
                    ...
                    <Badge
                      variant={viewingBook.available ? "default" : "secondary"}
                    >
                  ...
                  <div>
                    ...
                    <p className="text-sm">
                      {viewingBook.availableCopies} of {viewingBook.totalCopies}{" "}
                      available
                    </p>
                  </div>
                </div>
              ...
              <div className="flex gap-2">
                ...
                <Badge variant="outline">
                  {viewingBook.available ? "In Stock" : "Out of Stock"}
                </Badge>
              </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="font-semibold">
                      Available Problems ({viewingBook.problems.length})
                    </h4>
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() =>
                        handleViewProblems(
                          viewingBook as Book & { problems?: BookProblem[] },
                        )
                      }
                    >
                      <BookOpen className="mr-2 h-4 w-4" />
                    ...
                    {viewingBook.problems.slice(0, 3).map((problem) => (
                      <Card key={problem.id} className="p-3">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {problem.chapter} • Section {problem.section} •{" "}
      ...
      <Dialog
        open={!!viewingProblems}
        onOpenChange={() => setViewingProblems(null)}
      >
        <DialogContent className="max-h-[80vh] sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>Problems from: {viewingProblems?.title}</DialogTitle>
            <DialogDescription>
              Select problems to create assignments. {selectedProblems.length}{" "}
              problems selected.
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="space-y-4">
                  {viewingProblems?.problems
                    ?.filter(
                      (problem) =>
                        tab === "all" ||
                        problem.difficulty.toLowerCase() === tab,
                    )
                    .map((problem) => (
                      <Card
                        key={problem.id}
                        ...
                        <CardContent className="pt-0">
                          <div className="space-y-3">
                            ...
                            <div>
                              ...
                              <div className="whitespace-pre-line rounded bg-muted p-2 text-sm">
                                {problem.solution}
                              </div>
                            </div>
                          </div>
          ...
          <DialogFooter className="flex justify-between">
            ...
            <div className="space-x-2">
              <Button
                variant="outline"
                onClick={() => setViewingProblems(null)}
              >
                Cancel
              </Button>
              <Button
                onClick={handleCreateAssignmentFromProblems}
      ...
      >
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>
              Configure your assignment with the {selectedProblems.length}{" "}
              selected problems.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="max-h-48 overflow-y-auto rounded-lg border p-4">
              <h4 className="mb-2 font-medium">Selected Problems:</h4>
          ...
          <DialogFooter>
            ...
            <Button
              onClick={() => {
                ...
                console.log(
                  "Creating assignment with problems:",
                  selectedProblems,
                );
