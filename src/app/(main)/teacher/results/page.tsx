...
export default function ResultsPage() {
  ...
  const filteredResults = studentResults.filter((result) => {
    ...
    return groupMatch && assignmentMatch && studentMatch;
  });
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            {groupPerformance.slice(0, 6).map((group) => (
              <button
                type="button"
                key={group.groupName}
                className="flex w-full cursor-pointer items-center justify-between rounded-lg border p-4 transition-colors hover:bg-muted/50"
